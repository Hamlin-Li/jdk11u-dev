...
bool VirtualSpaceNode::should_commit_large_pages_when_reserving(size_t bytes) {
                                                ...
                                                SizeCounter* commit_words_counter)
{
  ...
  bool large_pages = should_commit_large_pages_when_reserving(word_size * BytesPerWord);
  ReservedSpace rs(word_size * BytesPerWord,
                   Settings::virtual_space_node_reserve_alignment_words() * BytesPerWord,
                   large_pages); // os::vm_page_size());
  if (!rs.is_reserved()) {
