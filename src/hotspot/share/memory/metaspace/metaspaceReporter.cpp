...
static const char* describe_spacetype(Metaspace::MetaspaceType st) {
  ...
  switch (st) {
    ...
    case Metaspace::AnonymousMetaspaceType: s = "ClassMirrorHolder"; break;
...
void MetaspaceReporter::print_report(outputStream* out, size_t scale, int flags) {
  ...
  const bool print_loaders = (flags & (int)ShowLoaders) > 0;
  const bool print_classes = (flags & (int)ShowClasses) > 0;
  const bool print_by_chunktype = (flags & (int)BreakDownByChunkType) > 0;
  const bool print_by_spacetype = (flags & (int)BreakDownBySpaceType) > 0;
