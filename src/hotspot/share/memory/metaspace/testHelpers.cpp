  ...
  _rs()
{
  ...
  if (reserve_limit > 0) {
    bool large_pages = VirtualSpaceNode::should_commit_large_pages_when_reserving(reserve_limit * BytesPerWord);
    // have reserve limit -> non-expandable context
    _rs = ReservedSpace(reserve_limit * BytesPerWord, Metaspace::reserve_alignment(), large_pages); //os::vm_page_size());
...
MetaspaceTestContext::~MetaspaceTestContext() {
  DEBUG_ONLY(verify();)
  MutexLockerEx fcl(Metaspace_lock, Mutex::_no_safepoint_check_flag);
  delete _context;
  if (_rs.is_reserved()) {
    _rs.release();
  }
...
MetaspaceTestArena* MetaspaceTestContext::create_arena(Metaspace::MetaspaceType type) {
  ...
  {
    MutexLockerEx ml(lock,  Mutex::_no_safepoint_check_flag);
