...
void MetaspaceDCmd::execute(DCmdSource source, TRAPS) {
  ...
  if (_basic.value() == true) {
    ...
    int flags = 0;
    if (_show_loaders.value())         flags |= (int)MetaspaceReporter::ShowLoaders;
    if (_show_classes.value())         flags |= (int)MetaspaceReporter::ShowClasses;
    if (_by_chunktype.value())         flags |= (int)MetaspaceReporter::BreakDownByChunkType;
    if (_by_spacetype.value())         flags |= (int)MetaspaceReporter::BreakDownBySpaceType;
    if (_show_vslist.value())          flags |= (int)MetaspaceReporter::ShowVSList;
